  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm transition-all duration-300">
        {isEditMode && (
          <div className="bg-[#2D2D7B] py-2 px-4 flex justify-between items-center">
            <div className="flex items-center gap-4">
              <span className="text-white text-xs font-black uppercase tracking-widest">Admin Panel Active</span>
              <button
                onClick={() => setShowPasswordChangeModal(true)}
                className="text-white/70 hover:text-white text-[10px] font-bold uppercase border border-white/20 px-3 py-1 rounded-full transition-colors"
              >
                Change Password
              </button>
            </div>
            <button
              ...
              className="bg-red-500 text-white px-4 py-1 rounded-full text-xs font-bold hover:bg-red-600 transition-colors flex items-center gap-2"
            >
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-20 md:h-24 flex items-center justify-between bg-white/80 backdrop-blur-md">
              ...
              {[-1, 0, 1].map((offset) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`relative flex-shrink-0 w-[260px] md:w-[400px] aspect-[4/3] rounded-3xl shadow-2xl bg-white border-4 ${isActive ? 'border-[#28A745]' : 'border-transparent cursor-pointer'}`}
                    onClick={() => {
                      if (!isActive) {
